<?php
$xiititl1 = "SELECT kelas_xii.jenis_kelas, tb_guru.nama_guru, tb_guru.kd_guru, mapel.nama_pelajaran FROM kelas_xii AS kelas_xii JOIN tb_guru AS tb_guru ON kelas_xii.kd_guru = tb_guru.kd_guru JOIN mapel AS mapel ON kelas_xii.kd_mapel = mapel.kd_mapel WHERE jenis_kelas = 'TITL-1'";
$xiititl1Data = mysqli_query($config, $xiititl1);
$subjects = array();

if ($xiititl1Data->num_rows > 0) {
    // Memasukkan data ke dalam array
    while ($row = $xiititl1Data->fetch_assoc()) {
    ...
    foreach ($times as $time) {
        ...
        if ($day == 'Senin' && $time == '07:40 - 08:05') {
            ...
            if (count($selectedSubjects) >= 3) {
                $subject = get_xiititl1($selectedSubjects); // Select from the already selected subjects
            } else {
                $subject = get_xiititl1(array_keys($subjects)); // Select from all subjects
...
<table class="tb-gen w-full text-[.85rem] text-[#666] font-medium pt-3">
    <thead class="text-white">
        <tr>
            <th colspan="4" class="bg-sky-600">XII TITL 1</th>
        </tr>
        <tr>
