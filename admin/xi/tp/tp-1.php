<?php
$xitp1 = "SELECT kelas_xi.jenis_kelas, tb_guru.nama_guru, tb_guru.kd_guru, mapel.nama_pelajaran FROM kelas_xi AS kelas_xi JOIN tb_guru AS tb_guru ON kelas_xi.kd_guru = tb_guru.kd_guru JOIN mapel AS mapel ON kelas_xi.kd_mapel = mapel.kd_mapel WHERE jenis_kelas = 'TITL-1'";
$xitp1Data = mysqli_query($config, $xitp1);
$subjects = array();

if ($xitp1Data->num_rows > 0) {
    // Memasukkan data ke dalam array
    while ($row = $xitp1Data->fetch_assoc()) {
    ...
    foreach ($times as $time) {
        ...
        if ($day == 'Senin' && $time == '07:40 - 08:05') {
            ...
            if (count($selectedSubjects) >= 3) {
                $subject = get_tp1($selectedSubjects); // Select from the already selected subjects
            } else {
                $subject = get_tp1(array_keys($subjects)); // Select from all subjects
...
<div class="mt-[4rem]">
    <h3 class="text-sky-600 font-bold text-2xl">TP</h3>
    <table class="tb-gen w-full text-[.85rem] text-[#666] font-medium pt-3 mt-5">
        <thead class="text-white">
            <tr>
                <th colspan="4" class="bg-sky-600">Kelas TP 1</th>
            </tr>
            <tr>
